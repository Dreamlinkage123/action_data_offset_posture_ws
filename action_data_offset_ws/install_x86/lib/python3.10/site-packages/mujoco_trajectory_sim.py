...
def run() -> None:
    p = argparse.ArgumentParser(description="MuJoCo 读取 MJCF + 关节轨迹 .data 回放（FK/IK: Pinocchio）")
    ...
    def tf_step(frame_idx: int) -> None:
        ...
        # Pinocchio 侧做 FK/IK，并把 7 关节结果回填到 MuJoCo qpos
        step_frame_ik_passes(
            tf_kins,
            tf_passes,
            q_pin,
