...
document.getElementById("btn-band-list-source")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const sel = document.getElementById("band-source-select");
...
document.getElementById("btn-band-produce")?.addEventListener("click", async () => {
  ...
  const sel = document.getElementById("band-source-select");
  const filenames = sel ? [...sel.selectedOptions].map((o) => o.value) : [];
  if (!sourceDir) {
    await modalAlert("请先填写数据来源目录（绝对路径）");
    return;
  }
  if (!filenames.length) {
    await modalAlert("请先在来源文件列表中选择至少一个文件（Ctrl/Cmd / Shift 多选）");
    return;
  }
...
document.getElementById("btn-band-play")?.addEventListener("click", async () => {
  const files = bandOrder.paths.slice();
  if (!files.length) {
  ...
  if (
    !(await modalConfirm(
      "将先开启「上半身调试模式」，再按所选顺序串接播放以下偏移数据（100Hz）。\n\n" +
        `共 ${files.length} 个片段。点击「确定」执行；「取消」则不执行。`
    ))
  ) {
    return;
  }
